...
export default function SalesOrdersPage() {
  ...
  const getPaymentBadge = (order: any) => {
    ...
    // Otgruzka qilinmagan, vozvrat qilingan yoki rad etilgan buyurtma haqiqiy
    // "qarzdorlik" emas (bitim yakunlanmagan yoki bekor qilingan) — shu sabab
    // qizil/sariq ogohlantiruvchi belgi emas, oddiy summa ko'rsatiladi.
    if (!order.is_shipped || order.status === 'Vozvrat qilindi' || order.status === 'Rad etildi') {
  ...
  const visibleOrders = orders.filter(order => {
    if (debtOnly) {
      ...
      if (!order.is_shipped || order.status === 'Vozvrat qilindi' || order.status === 'Rad etildi') return false;
      const ps = paymentStatus[order.id];
      const remaining = ps ? Number(ps.remaining_uzs) : Number(order.total_uzs_price);
